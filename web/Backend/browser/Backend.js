class Backend {
  ...
  render(targetElement) {
      ...
      // App Selector Dropdown with File Searcher added
      this.appSelector = makeElement('select', {
        ...
        makeElement('option', { value: 'searcher', selected: this.activeApp === 'searcher' }, '🔍 File Searcher')
      ]);
      ...
      // Form C: File Searcher Panel
      this.searchDirInput = makeElement('input', { type: 'text', value: '/Users/rob' });
      this.searchTypeSelect = makeElement('select', {
        onchange: (e) => {
          const val = e.target.value;
          this.searchContentGroup.style.display = (val === 'content' || val === 'both') ? 'block' : 'none';
        }
      }, [
        makeElement('option', { value: 'name' }, 'File Name Only'),
        makeElement('option', { value: 'content' }, 'File Content Only'),
        makeElement('option', { value: 'both' }, 'Name Wildcard & Content Match')
      ]);
      this.searchNamePatternInput = makeElement('input', { type: 'text', value: '*' });
      this.searchContentPatternInput = makeElement('input', { type: 'text', value: '', placeholder: 'e.g. search string' });
      this.searchCaseSensitiveCheck = makeElement('input', { type: 'checkbox' });
      this.searchSkipDirsInput = makeElement('textarea', {}, 'node_modules, .git, .cache, Library, System, Applications, .Trash');
      this.searchSkipHiddenCheck = makeElement('input', { type: 'checkbox', checked: true });

      this.searchContentGroup = makeElement('div', { className: 'form-group', style: { display: 'none' } }, [
        makeElement('label', {}, 'Text Content to Search:'),
        this.searchContentPatternInput
      ]);

      this.searcherPanel = makeElement('div', { className: 'panel-box', style: { display: 'none' } }, [
        makeElement('h2', { className: 'panel-title' }, 'File Searcher'),
        makeElement('div', { className: 'form-group' }, [makeElement('label', {}, 'Starting Path:'), this.searchDirInput]),
        makeElement('div', { className: 'form-group' }, [makeElement('label', {}, 'Search Type:'), this.searchTypeSelect]),
        makeElement('div', { className: 'form-group' }, [makeElement('label', {}, 'File Name Pattern (Wildcard):'), this.searchNamePatternInput]),
        this.searchContentGroup,
        makeElement('div', { className: 'checkbox-row', style: { marginBottom: '10px' } }, [
          makeElement('label', { className: 'checkbox-item' }, [this.searchCaseSensitiveCheck, 'Case Sensitive Search']),
          makeElement('label', { className: 'checkbox-item' }, [this.searchSkipHiddenCheck, 'Skip Hidden Folders'])
        ]),
        makeElement('div', { className: 'form-group' }, [makeElement('label', {}, 'Skip Directories:'), this.searchSkipDirsInput]),
        makeElement('div', { className: 'btn-bar' }, [
          this.searchStartBtn = makeElement('button', { className: 'btn btn-success', onclick: () => this.startFileSearch() }, 'Search'),
          this.searchStopBtn = makeElement('button', { className: 'btn btn-danger btn-disabled', onclick: () => this.stopFileSearch() }, 'Stop')
        ])
      ]);
      sideStack.appendChild(this.searcherPanel);
  ...
  switchApp(appName) {
      ...
      if (appName === 'deployer') {
        ...
        this.searcherPanel.style.display = 'none';
        this.renderProjectsList();
      } else if (appName === 'scanner') {
        this.deployPanel.style.display = 'none';
        this.scannerPanel.style.display = 'block';
        this.searcherPanel.style.display = 'none';
        this.renderFileList();
      } else if (appName === 'searcher') {
        this.deployPanel.style.display = 'none';
        this.scannerPanel.style.display = 'none';
        this.searcherPanel.style.display = 'block';
  ...
  async pollStatus() {
      try {
        // Poll scanner status
        const scanData = await this.runRemote(async (env) => {
          const ScannerClass = await env.getClass('BigFileScanner');
          const scanner = env.getScannerInstance(ScannerClass);
          return {
        ...
        // Poll searcher status
        const searchData = await this.runRemote(async (env) => {
          const SearcherClass = await env.getClass('FileSearcher');
          if (!global.searcherInstance) {
            global.searcherInstance = new SearcherClass();
          }
          const searcher = global.searcherInstance;
          return {
            searching: searcher.searching,
            progress: searcher.progress,
            scanned: searcher.filesSearchedCount,
            found: searcher.matchesFoundCount,
            results: searcher.results
          };
        });

        this.serverActive = true;
        this.isScanning = scanData.result.scanning;
        this.isSearchingFiles = searchData.result.searching;

        // Update active badge and inputs state
        if (this.isScanning) {
          this.badge.className = 'server-badge badge-scanning';
          this.badge.textContent = 'SCANNING (' + scanData.result.scanned.toLocaleString() + ')';
          this.startBtn.classList.add('btn-disabled');
          this.stopBtn.classList.remove('btn-disabled');
        } else if (this.isSearchingFiles) {
          this.badge.className = 'server-badge badge-scanning';
          this.badge.textContent = 'SEARCHING (' + searchData.result.scanned.toLocaleString() + ')';
          this.searchStartBtn.classList.add('btn-disabled');
          this.searchStopBtn.classList.remove('btn-disabled');
        } else {
          ...
          this.searchStartBtn.classList.remove('btn-disabled');
          this.searchStopBtn.classList.add('btn-disabled');
        }

        this.currentResults = scanData.result.results || [];
        this.currentSearchResults = searchData.result.results || [];

        if (this.activeApp === 'scanner') {
          this.renderFileList(scanData.result.scanned, scanData.result.progress);
        } else if (this.activeApp === 'searcher') {
          this.renderSearchResults(searchData.result.scanned, searchData.result.progress);
        }
      } catch(e) {
        this.serverActive = false;
        this.isScanning = false;
  ...
  renderFileList(scannedCount = 0, progressText = '') {
      if (this.activeApp !== 'scanner') return;
      this.resultsWrapper.innerHTML = '';

      if (this.isScanning) {
        const progressBanner = makeElement('div', {
          style: {
          ...
          makeElement('strong', {}, '📂 SCANNED DIRECTORIES ACTIVE PROGRESS...'),
          makeElement('div', { style: { marginTop: '4px', color: '#94a3b8' } }, `Files Checked: ${scannedCount.toLocaleString()}`),
          makeElement('div', { style: { marginTop: '2px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', color: '#cbd5e1' } }, `${progressText || 'Scanning...'}`)
        ]);
        this.resultsWrapper.appendChild(progressBanner);
      }
  ...
  async startFileSearch() {
      if (!this.serverActive || this.isSearchingFiles) return;
      this.deletedFiles.clear();
      this.currentSearchResults = [];

      const directory = this.searchDirInput.value.trim();
      const searchType = this.searchTypeSelect.value;
      const namePattern = this.searchNamePatternInput.value.trim();
      const contentPattern = this.searchContentPatternInput.value;
      const caseSensitive = this.searchCaseSensitiveCheck.checked;
      const skipDirs = this.searchSkipDirsInput.value.trim();
      const skipHidden = this.searchSkipHiddenCheck.checked;

      this.appendLog(`Triggering background file search in directory: ${directory}...`);
      ...
      try {
        await this.runRemote(async (env) => {
          ...
          searcher.startSearch({
            directory: env.params.directory,
            searchType: env.params.searchType,
            namePattern: env.params.namePattern,
            contentPattern: env.params.contentPattern,
            caseSensitive: env.params.caseSensitive,
            skipDirs: env.params.skipDirs,
            skipHidden: env.params.skipHidden
          });
          return { status: 'started' };
        }, { directory, searchType, namePattern, contentPattern, caseSensitive, skipDirs, skipHidden });

        this.pollStatus();
      } catch(e) {
        this.appendLog("Search launch failed: " + e.message);
      }
    }

  async stopFileSearch() {
      if (!this.serverActive || !this.isSearchingFiles) return;
      this.appendLog("Sending interrupt command to File Searcher...");
      try {
        await this.runRemote(async (env) => {
          if (global.searcherInstance) {
            global.searcherInstance.abortRequested = true;
          }
          return { status: 'aborting' };
        });
        this.pollStatus();
      } catch(e) {
        this.appendLog("Abort request failed: " + e.message);
      }
    }

  renderSearchResults(scannedCount = 0, progressText = '') {
      if (this.activeApp !== 'searcher') return;
      this.resultsWrapper.innerHTML = '';
      ...
      if (this.isSearchingFiles) {
        const progressBanner = makeElement('div', {
          ...
          makeElement('strong', {}, '🔍 ACTIVE SEARCH IN PROGRESS...'),
          makeElement('div', { style: { marginTop: '4px', color: '#94a3b8' } }, `Files Checked: ${scannedCount.toLocaleString()}`),
          makeElement('div', { style: { marginTop: '2px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', color: '#a7f3d0' } }, `${progressText || 'Scanning directory structure...'}`)
        ]);
        this.resultsWrapper.appendChild(progressBanner);
      } else if (scannedCount > 0) {
        const completionBanner = makeElement('div', {
          style: {
        ...
        }, `✓ Search complete. Evaluated ${scannedCount.toLocaleString()} files. Found ${this.currentSearchResults.length} matches.`);
        this.resultsWrapper.appendChild(completionBanner);
      }

      const header = makeElement('h3', { style: { color: '#38bdf8', marginTop: '0' } }, `Matches Found (${this.currentSearchResults.length})`);
      this.resultsWrapper.appendChild(header);
      ...
      if (this.currentSearchResults.length === 0) {
        this.resultsWrapper.appendChild(
          makeElement('div', { style: { color: '#64748b', padding: '40px', textAlign: 'center' } }, 'No matches found yet. Fill in search terms and click "Search".')
        );
        return;
      }

      this.currentSearchResults.forEach(file => {
        const isDeleted = this.deletedFiles.has(file.path);
        const cardClass = `file-card ${isDeleted ? 'deleted' : ''}`;
        ...
        if (isDeleted) {
          ...
          actionCell.appendChild(makeElement('button', { className: 'btn-mini btn-reveal', onclick: (e) => { e.stopPropagation(); this.revealFile(file.path); } }, 'Reveal'));
          actionCell.appendChild(makeElement('button', { className: 'btn-mini btn-delete', onclick: (e) => { e.stopPropagation(); this.deleteFile(file.path); } }, 'Delete'));
        }

        const snippetEl = file.snippet 
          ? makeElement('div', { style: { fontFamily: 'monospace', fontSize: '11px', color: '#a7f3d0', backgroundColor: '#090d16', padding: '6px', borderRadius: '4px', marginTop: '6px', whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, file.snippet)
          : null;

        const card = makeElement('div', { 
          className: cardClass,
          style: { cursor: 'pointer' },
          onclick: () => this.revealFile(file.path)
        }, [
          makeElement('div', { className: 'file-info' }, [
            makeElement('div', { className: 'file-name', style: { color: '#38bdf8' } }, fileName),
            makeElement('div', { className: 'file-path' }, dirPath),
            makeElement('div', { className: 'file-meta' }, `${file.matchType} | ${file.sizeMB} MB | ${dateStr}`),
            snippetEl
          ].filter(Boolean)),
          actionCell
        ]);
